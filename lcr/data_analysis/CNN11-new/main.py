...
def convert_np_to_dssims(np_arrays, titles):
    ...
    for i in range(len(np_arrays)):
        ...
        dssims_da = dssims_da.roll(dim_1=144, roll_coords=False)
...
if __name__ == "__main__":
    ...
    for i in range(n):
        errors, av_preds, av_dssims, predictions, errors1, av_preds1, av_dssims1, errors3, av_preds3, av_dssims3 = build_and_evaluate_models_for_time_slices(time, j.split(".")[0], j.split(".")[0])
    ...
    print (time)
    print (errors_all)
    generate_performance_plots(test_slices, [errors_all, errors1_all, errors3_all],
    ...
    for cdir in cdirs:
        if type(time) == list:
            for t in time:
                ...
                ldcpy.plot(allthings, "dssims", calc="mean", sets=["Actual DSSIMs", "Model Predictions", "Error"], weighted=False, start=0, end=0, short_title=True, cmax=1, cmin=0, vert_plot=True)
