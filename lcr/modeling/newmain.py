...
def main():
    ...
    if modeltype == 'rf':
        ...
        random.seed(1)
        rf_model = RandomForestClassifier(n_estimators=100, max_depth=10, random_state=1, min_samples_leaf=10)
    ...
    if modeltype == "cnn":
        ...
        logging.info(f"Time taken for data preparation: {end_time - start_time}")
        try:
            train_data_np = np.load(f"{storageloc}/train_data_{j}{time}{modeltype}{jobid}.npy", allow_pickle=False)
        except Exception as e:
...
def compute_features(data_xr, featurelist, storage_loc="./data", varname="combined", orig_label="orig", m="metric", times=None):
    ...
    for feature in featurelist:
        ...
        #     feat_da = xr.open_dataarray(f"{storage_loc}/all_big_combined_{orig_label}_FEATURE_{feature}_all_time2000_second.nc")
        #
        ...
        if os.path.exists(f"{storage_loc}/{varname}_{orig_label}_FEATURE_{feature}_{m}_time2000_second.nc"):
            logging.info(f"Loading cached feature from {storage_loc}/{varname}_{orig_label}_FEATURE_{feature}_{m}_time2000_second.nc: {feature}")
            feat_da = xr.open_dataarray(f"{storage_loc}/{varname}_{orig_label}_FEATURE_{feature}_{m}_time2000_second.nc")
