...
def optimal_level_min(csvfilename, variable, threshold, compression, freq):
    ...
    levs = []
    for time in times:
        lev = optimal_level(f"../data/test_set/{freq}_dssims.csv", variable, time, threshold, compression)
        levs.append(lev)
    min_level = min(levs)
    return min_level
...
def create_daily_monthly_freq_hist():
    for freq in ['daily', 'monthly']:
        ...
        for varname in v:
            level = optimal_level_spread(f"../data/test_set/{freq}_dssims.csv", varname, 0.9995, "bg", freq)
