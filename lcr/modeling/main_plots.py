        ...
        if labelsonly:
            ...
            for t in time:
                ...
                preds_cnn[t] = load_first_existing(
                    [
                        f"{storageloc}/test_predictions_02000cnn0_{vlist[0]}.npy",
                        f"{storageloc}/test_predictions_12000cnn0_{vlist[0]}.npy",
                    ],
                    allow_pickle=True
                )
...
# and the big chart
            ...
            with open(f"{storageloc}/confusion_matrix_{metric}_{i}_{j.split('.')[0]}{jobid}{model}_{date_string}_CONVOLUTIONAL_{flat_vlist}.txt", 'w') as f:
                f.write(str(cm_cnn))

            with open(f"{storageloc}/classification_report_{metric}_{i}_{j.split('.')[0]}{jobid}{model}_{date_string}_CONVOLUTIONAL_{flat_vlist}.txt", 'w') as f:
                f.write(report_cnn)

            np.save(f"{storageloc}/confusion_matrix_{metric}_{i}_{j.split('.')[0]}{jobid}rf_{date_string}_{flat_vlist}.npy", cm_rf)
            ind_rf = list(set(np.append(classifyp_rf, classifyd)))
            cm_rf = pd.DataFrame(cm_rf, index=ind_rf, columns=ind_rf)
            with open(f"{storageloc}/confusion_matrix_{metric}_{i}_{j.split('.')[0]}{jobid}{model}_{date_string}_RANDOMFOREST_{flat_vlist}.txt", 'w') as f:
                f.write(str(cm_rf))

            with open(f"{storageloc}/classification_report_{metric}_{i}_{j.split('.')[0]}{jobid}{model}_{date_string}_RANDOMFOREST_{flat_vlist}.txt", 'w') as f:
                f.write(report_rf)
        ...
        file_name = f"{storageloc}/result_table_{metric}_{i}_{j.split('.')[0]}{jobid}{model}_{date_string}_{flat_vlist}.csv"
        ...
        # for i in time:
            ...
            cm_cnn = confusion_matrix(classifyd, classifyp_cnn, labels=sorted(list(set(np.append(classifyp_cnn, classifyd)))))
            cm_rf = confusion_matrix(classifyd, classifyp_rf, labels=sorted(list(set(np.append(classifyp_rf, classifyd)))))

            # reorder the matrix

            # Get the unique labels
            unique_labels = sorted(np.unique(np.concatenate([classifyd, classifyp_cnn, classifyp_rf])).tolist())
