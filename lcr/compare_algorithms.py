...
def optimal_per_slice():
    freqs = ["daily"]
    for freq in freqs:
        csvfilename = f"../data/all_daily_24/{freq}_zfp_bg_sz_comp_slices.csv"
        ...
        for rowstring in rowstrings:
            ...
            if np_ratios.argmax(0) == 0:
                ...
                best_alg = "z_hdf5"
            location = f"../data/all_daily_24/{freq}_optimal_slices.csv"
...
def optimal_over_var():
    freqs = ["daily", "monthly"]
    for freq in freqs:
        csvfilename = f"../data/all_daily_24/{freq}_zfp_bg_sz_comparison.csv"
        with open(csvfilename, newline='') as csvfile:
            content = csvfile.readlines()
        rowstrings = content[1:]
        for rowstring in rowstrings:
            ...
            if np_ratios.argmax(0) == 0:
                ...
                best_alg = "z_hdf5"
            location = f"../data/all_daily_24/{freq}_optimal_over_var.csv"
            file_exists = os.path.isfile(location)
            with open(location, 'a', newline='') as newcsvfile:
                fieldnames = ["variable", "best_alg", "best_ratio", "best_size", "best_level"]
                writer = csv.DictWriter(newcsvfile, fieldnames=fieldnames)
