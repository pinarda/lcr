...
foreach x ($arrname)
  cat batch_script_zfp_template.sh | sed "s/MATCHME.csv/daily$x.csv/" | sed "s/MATCHME.json/$x.json/" > zfp/$x.sh
  cat sample_zfp_diff_config.json | sed "s/MATCHME/$x/" > zfp/$x.json
  cat batch_script_sz_template.sh | sed "s/MATCHME.csv/daily$x.csv/" | sed "s/MATCHME.json/$x.json/" > sz/$x.sh
  cat sample_sz_diff_config.json | sed "s/MATCHME/$x/" > sz/$x.json
  cat batch_script_br_template.sh | sed "s/MATCHME.csv/daily$x.csv/" | sed "s/MATCHME.json/$x.json/" > br/$x.sh
  cat sample_br_diff_config.json | sed "s/MATCHME/$x/" > br/$x.json
  cat batch_script_orig_template.sh | sed "s/MATCHME.csv/daily$x.csv/" | sed "s/MATCHME.json/$x.json/" > orig/$x.sh
