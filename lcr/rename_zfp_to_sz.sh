...
foreach x ($arrname)
  ...
  cat batch_scripts/cheyenne_batch_$x.sh | sed "s/-j '.\/batch_scripts\/$x.json'/-j '.\/batch_scripts\/sz_dssim_scripts\/$x.json'/" | sed "s/'\/glade\/scratch\/apinard\/$x\_calcs.csv'/'\/glade\/scratch\/apinard\/sz3\/$x\_calcs.csv'/" > batch_scripts/sz_dssim_scripts/cheyenne_batch_$x.sh
end